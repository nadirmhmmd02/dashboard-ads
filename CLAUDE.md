...
app/
  ...
  components/
    ...
    mapsSync.js          → mesin sync Maps Hub PURE (tanpa React/DB — bisa dites Node): parseCsv gviz, parseOrdinat (6 varian nyata: normal/koma-desimal/DMS derajat/kurung/spasi/koma-buntut + validasi rentang Indonesia lat -11..6 lng 94..142; Plus Code Google TIDAK didukung → Needs Review), kotaFromAlamat (KOTA HYBRID per 21 Agu 2026: dari ALAMAT dulu — pola Kota/Kabupaten/Kab./"X Regency,"/"X City," + alias SBY/Bks/Jaksel + Inggris West Jakarta→Jakarta Barat; 379/486 outlet; sisanya fallback OSM tingkat kota/kabupaten via KOTA_VERSION reset otomatis di route), outletsFromCsv (baca kolom BY NAMA HEADER, duplikat dikeluarkan dari diff), diffOutlets (Baru/Relokasi via Nama Lama/Perubahan Info/Hilang; field pemicu alert perubahan HANYA alamat+ordinat_raw+nama_gmaps; sync pertama tanpa alert). 31 tes node di scratchpad sudah pass dgn CSV asli.
    ...
    (SuggestionsModal.js, BarChart.js, Navbar.js SUDAH DIHAPUS 26 Agu 2026 — file lama tak terpakai; logika Suggestions ada di AppShell + page.js. Kalau butuh, ambil dari riwayat Git.)
