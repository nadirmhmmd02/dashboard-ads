...
app/
  ...
  supabase-notes-setup.sql (root repo) → SQL tabel `notes` (catatan pribadi + RLS per pemilik + trigger updated_at). **STATUS: BELUM DIJALANKAN Nadir per 20 Jul 2026** — sampai dijalankan, panel Notes menampilkan pesan "Notes is not set up yet…".
