...
app/
  ...
  leads/page.js          → LEADS HUB Dashboard v3.0 (LIVE, English UI): KPI 3 kartu (New Leads/Follow-up %/Total Closing) + 5 KARTU STATUS besar + donut Status Distribution (angka tabular-nums lurus) + Cost&ROI (spend konversi, CPD, ROAS=closing/spend, ROI=(closing−spend)/spend, cohort by created_at) + Sales Performance (per sales, warna khas) + By Category. Filter tanggal (useLeadsFilter) + kategori (Dropdown custom rata kanan). Spend via GET /api/leads?mode=spend. Banner Black Box admin.
  leads/list/page.js     → LEADS HUB Leads List v3.0 (LIVE, English UI): tab "BLACK BOX" admin (approve/reject, bulk context-aware) + All Leads. Toolbar urut: Black Box · search · All Leads (SPLIT BUTTON: klik nama=pindah tab, klik panah=filter kategori) · All Status · All Sales (di Black Box tampil disabled cursor not-allowed) · Columns (hide/show kolom, persist localStorage wd-leads-cols-hidden). Kolom: Date/Name(+label NEW merah, hilang setelah aksi apa pun via is_new)/Phone(copy)/Email/City(domicile)/Category/Campaign/Sales(warna per orang: Akmel biru, Hendra ungu, Dedik teal)/Status pill/FU/Notes/Closing; data kosong = "-". Popup Deal WAJIB nominal (deal_reason = input teks bebas opsional). Bulk: copy, FU, set status non-Deal, assign sales. Tombol "Add Leads" dropdown (Sync Meta Leads + Import from File placeholder). Header tabel kiri (kolom aksi center), row entrance stagger wdFadeUp. RLS: marketing hanya lead approved, viewer read-only.
