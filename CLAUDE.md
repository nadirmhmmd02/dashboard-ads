...
app/
  ...
  maps/page.js           → MAPS HUB "Outlet Maps" (ADMIN-ONLY, dibangun 21 Agu 2026 — konteks & keputusan: MAPS-HUB-PLAN.md): monitoring status Google Maps 500+ outlet dari Google Sheets read-only. Atas→bawah: header card (Wilayah/Refresh/Sync/theme) → 5 KPI klik=filter status (Total/Registered/Not Registered/Needs Claim/Needs Review) → banner geocode (kalau ada kota kosong; loop klien batch 35, progress bar, bisa Stop) → peta Leaflet cluster (flex 2.6) + panel tab Alerts|Data Quality → tabel outlet + toolbar filter (search/Depo/Provinsi/Kota, reset). Klik baris tabel → peta flyTo + buka popup. Modal Depo→Provinsi: baris depo expand → CHIP provinsi (bukan Dropdown — anti clipping di modal scroll); depo belum dipetakan paling atas + badge oranye di tombol Wilayah. Outlet missing_since: baris redup + badge MISSING, tidak dirender di peta. needsSetup (tabel belum ada) → kartu hint jalankan supabase-maps-setup.sql. Butuh env MAPS_SHEET_ID (.env.local + Vercel).
  ...
  components/
    ...
    mapsConfig.js        → registry Maps Hub: MAPS_STATUS (nilai HARUS sama dgn dropdown "Profile Bisnis" di sheet) + warna, 38 PROVINSI, tile CARTO light/dark + atribusi, statusColor().
    mapsSync.js          → mesin sync Maps Hub PURE (tanpa React/DB — bisa dites Node): parseCsv gviz, parseOrdinat (6 varian nyata: normal/koma-desimal/DMS derajat/kurung/spasi/koma-buntut + validasi rentang Indonesia lat -11..6 lng 94..142; Plus Code Google TIDAK didukung → Needs Review), outletsFromCsv (baca kolom BY NAMA HEADER, duplikat dikeluarkan dari diff), diffOutlets (Baru/Relokasi via Nama Lama/Perubahan Info/Hilang; field pemicu alert perubahan HANYA alamat+ordinat_raw+nama_gmaps; sync pertama tanpa alert). 31 tes node di scratchpad sudah pass dgn CSV asli.
    MapView.js           → peta Leaflet MURNI (BUKAN react-leaflet — dynamic import di useEffect, SSR aman, StrictMode-safe via cleanup). Cluster leaflet.markercluster (iconCreateFunction custom .wd-cluster), marker divIcon titik warna status, popup theme-aware (styling globals.css .wd-map/.wd-pop), tile setUrl saat ganti tema, fitBounds saat filter berubah, prop focus → flyTo+openPopup (zoomToShowLayer utk keluar dari cluster).
  ...
  api/maps/route.js      → API Maps Hub (SEMUA admin-only, maxDuration 60): GET = outlets+alerts+mapping+lastSync (deteksi tabel belum dibuat → needsSetup). POST action=sync (fetch gviz CSV via env MAPS_SHEET_ID → mapsSync.js → tulis batch: insert bulk / upsert onConflict nama / rename satu-satu; alert anti-dobel per outlet+type open; reappeared → alert hilang auto-done; log ringkasan ke maps_sync_log), action=mark_done, action=set_provinsi, action=geocode (Nominatim reverse batch ≤35 × 1.1 detik + cache maps_geocode_cache; kota "-" = sudah dicoba tak ketemu; 429 → stop putaran; klien memanggil berulang sampai remaining 0). Sheet TIDAK PERNAH ditulis.
...
- [ ] **MAPS HUB — KODE SUDAH DIBANGUN & DI-PUSH 21 Agu 2026** (halaman /maps + /api/maps + supabase-maps-setup.sql; keputusan lengkap di `MAPS-HUB-PLAN.md`). **Tinggal 2 langkah manual Nadir:** (1) jalankan `supabase-maps-setup.sql` di Supabase SQL Editor (sekali); (2) tambah env `MAPS_SHEET_ID` di Vercel → Settings → Env Variables (nilai sama dgn di `.env.local`) + Redeploy. Setelah itu: buka /maps → Sync → isi mapping Depo→Provinsi (tombol Settings di header) → klik "Geocode now" (sekali, ±8 menit, boleh di-Stop & dilanjut). Belum dites end-to-end di production sampai 2 langkah itu jalan.
