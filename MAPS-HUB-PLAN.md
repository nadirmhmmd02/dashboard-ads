...
- **Kota — REVISI 21 Agu 2026 (keputusan Nadir, hasil geocode v1 salah tingkat = kecamatan): HYBRID.** (1) Utama: ekstrak dari kolom ALAMAT via `kotaFromAlamat()` — pola "Kota X"/"Kabupaten X"/"Kab. X"/"X Regency,"/"X City," + kamus singkatan (SBY→Surabaya, Bks→Bekasi, Jaksel…) + format Inggris (West Jakarta→Jakarta Barat); mencakup 379/486 outlet, instan, kendali tetap di sheet (benerin alamat → sync → kota ikut). (2) Fallback: reverse-geocode OSM untuk sisanya dengan prioritas city→county→state_district→town (JANGAN municipality/village = kecamatan). Tingkat = kota/kabupaten; Jakarta dipecah per wilayah. `KOTA_VERSION` di route: naikkan kalau aturan berubah → sync berikutnya otomatis reset cache + kota lama (tanpa migrasi).
  Catatan lama: reverse-geocode Nominatim (OSM) **1x via SCRIPT LOKAL saat development** (BUKAN serverless — 500 req × 1 detik ≈ 9 menit > limit 60 detik Vercel; keputusan Nadir opsi A), hasil cache di Supabase. Outlet baru berikutnya di-geocode maks ±30 per sync (muat di 60s). Rate limit 1 req/detik + User-Agent jelas + atribusi OpenStreetMap WAJIB.
