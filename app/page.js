    ...
    <div style={{ flex:1, minHeight:0, display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ flex:1, minHeight:0, display:'flex', flexDirection:'column', padding:'16px 20px', gap:'12px', overflow:'hidden' }}>
        ...
        {!loading && !error && summary && (
          <>
            {/* ── 5 BIG CARDS ── */}
            <div style={{ flexShrink:0, display:'grid', gridTemplateColumns:'repeat(5, minmax(0,1fr))', gap:'10px', height:'130px' }}>
              {BIG_CARDS.map((c, i) => (
                ...
                >
                  <div style={{ display:'flex', alignItems:'center', gap:'6px', marginBottom:'10px' }}>
                    ...
                    <span style={{ fontSize:'11px', color:'var(--t2)', fontWeight:'500' }}>{c.label}</span>
                  </div>
                  <div style={{ fontSize:'26px', fontWeight:'700', color:'var(--t1)', lineHeight:1, marginBottom:'7px', letterSpacing:'-0.8px' }}>
            ...
            {/* ── 4 SMALL CARDS ── */}
            <div style={{ flexShrink:0, display:'grid', gridTemplateColumns:'repeat(4, minmax(0,1fr))', gap:'10px', height:'66px' }}>
              {SMALL_CARDS.map((c) => (
                <div key={c.label} style={{
                  background:'var(--cd)', border:'1px solid var(--br)',
                  borderRadius:'12px', padding:'0 18px',
                  display:'flex', alignItems:'center', justifyContent:'space-between',
                  height:'100%',
                }}>
                  <div>
                    <div style={{ fontSize:'10px', color:'var(--t3)', marginBottom:'4px', fontWeight:'500' }}>{c.label}</div>
                    <div style={{ fontSize:'18px', fontWeight:'600', color:'var(--t1)', letterSpacing:'-0.3px' }}>{c.value}</div>
                  </div>
